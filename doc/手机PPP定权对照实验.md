...
早期结果中 `out-outsingle=on`，`.pos` 同时含有 `Q=5` 单点解。部分单点解出现数百千米异常误差，因此不能与 PPP 解混合统计。该问题不影响下面已经按共同 `Q=6` 历元重新统计的结果。2026-09-03 起 `evaluate_ppp.py` 在脚本内强制过滤（PPP 只统计 `Q=6`，SPP 单点用 `--quality 5`），过滤先于时间匹配、共同历元筛选和完整时间轴插值，防止 Q=5 行作为直接解或插值端点污染分数（详见 `手机PPP研究进展.md` §5 与更新记录）。
